...
export class DevSeedService implements OnModuleInit {
  ...
  private async seedDevTenant() {
    ...
    this.logger.log('Dev seed: creando tenant "demo" con datos de demostración...');
    ...
    this.logger.log('✔ Dev seed completado con datos de demostración completos.');
    this.logger.log('  → Slug:          demo');
    this.logger.log(`  → Admin:         ${seedEmail} / (ver DEV_SEED_PASSWORD)`);
    ...
    this.logger.log('  → Inquilino 2:   carlos.mamani@gmail.com / Inquilino365!');
  ...
  private async seedUsers(schema: string) {
    ...
    if (empleado.length > 0) {
      const modules = [
        { module: 'properties', can_view: true, can_create: true, can_edit: true, can_delete: false },
        { module: 'contracts', can_view: true, can_create: false, can_edit: false, can_delete: false },
        { module: 'payments', can_view: true, can_create: true, can_edit: false, can_delete: false },
        { module: 'maintenance', can_view: true, can_create: true, can_edit: true, can_delete: false },
        { module: 'reports', can_view: true, can_create: false, can_edit: false, can_delete: false },
      ];
  ...
  private async seedRentalOwners(schema: string) {
    ...
    for (const o of owners) {
      await this.dataSource.query(
        ...
        [
          o.name, o.company_name, o.is_company, o.primary_email, o.phone_number, o.notes,
          o.bank_name, o.account_number, o.account_type, o.account_holder_name, o.cbu_iban,
        ],
      );
    }
  ...
  private async seedProperties(schema: string) {
    ...
    if (!residential || !commercial) {
      this.logger.warn('Dev seed: property_types no encontrados, omitiendo propiedades.');
      return;
    }

    const subtypes: { id: number; code: string }[] = await this.dataSource.query(
      `SELECT id, code FROM ${quoteIdent(schema)}.property_subtypes`,
    );
    const sub = (code: string) => subtypes.find((s) => s.code === code)?.id ?? 1;
    ...
    const amenitiesResidencial = JSON.stringify(['Agua caliente', 'Gas natural', 'Parqueo', 'Seguridad 24h', 'Áreas verdes']);
    const amenitiesComercial = JSON.stringify(['Estacionamiento', 'Acceso discapacitados', 'Aire acondicionado', 'Generador']);

    const properties = [
      {
        title: 'Departamento Moderno Zona Sur',
        description: 'Amplio departamento de 2 dormitorios con vista panorámica en Calacoto. Totalmente amoblado, cocina equipada y balcón.',
        ...
        address: { street: 'Av. Ballivián #1234, Piso 7', city: 'La Paz', state: 'La Paz', zip: '0000', country: 'BO' },
        owner_idx: 0,
      },
      {
        title: 'Casa Unifamiliar Achumani',
        description: 'Casa de 3 dormitorios con jardín en Achumani. Cocina americana, sala-comedor integrados y garage para 2 vehículos.',
        type_id: residential.id,
        ...
        year_built: 2015,
        is_furnished: false,
        amenities: JSON.stringify(['Jardín', 'Garage', 'Cuarto de servicio', 'Terraza']),
        security_deposit: 14400.00,
        rental_type: 'LONG_TERM',
        address: { street: 'Calle 15 de Achumani #567', city: 'La Paz', state: 'La Paz', zip: '0000', country: 'BO' },
        owner_idx: 0,
      },
      {
        title: 'Oficina Ejecutiva Sopocachi',
        description: 'Oficina de 60m² en pleno centro de Sopocachi. Incluye sala de reuniones compartida y recepción.',
        ...
        security_deposit: 7600.00,
        rental_type: 'LONG_TERM',
        address: { street: 'Av. 6 de Agosto #2100, Piso 4', city: 'La Paz', state: 'La Paz', zip: '0000', country: 'BO' },
        owner_idx: 1,
      },
      {
        title: 'Departamento Estudio Centro',
        description: 'Estudio compacto ideal para estudiantes o profesionales. Amoblado, internet incluido, edificio con vigilancia.',
        type_id: residential.id,
        subtype_id: sub('CONDO_TOWNHOME'),
        status: 'DISPONIBLE',
        ...
        year_built: 2019,
        is_furnished: true,
        amenities: JSON.stringify(['Internet', 'Seguridad', 'Lavandería común']),
        security_deposit: 4400.00,
        rental_type: 'BOTH',
        address: { street: 'Calle Comercio #890, Piso 3', city: 'La Paz', state: 'La Paz', zip: '0000', country: 'BO' },
        owner_idx: 0,
      },
      {
        title: 'Local Comercial Miraflores',
        description: 'Local de 90m² en avenida de alto tráfico. Ideal para retail, farmacia o restaurante. Entrega inmediata.',
        type_id: commercial.id,
        ...
        year_built: 2010,
        is_furnished: false,
        amenities: JSON.stringify(['Acceso discapacitados', 'Vitrina frontal', 'Bodega trasera']),
        security_deposit: 11000.00,
        rental_type: 'LONG_TERM',
        address: { street: 'Av. Saavedra #3400, Local 5', city: 'La Paz', state: 'La Paz', zip: '0000', country: 'BO' },
        owner_idx: 1,
      },
    ];
    ...
    for (let i = 0; i < properties.length; i++) {
      ...
      const inserted: { id: number }[] = await this.dataSource.query(
        `INSERT INTO ${quoteIdent(schema)}.properties
         ...
         RETURNING id`,
        [
          p.title, p.description, p.type_id, p.subtype_id, p.status,
          p.monthly_rent, p.square_meters, p.bedrooms, p.bathrooms, p.parking_spaces,
          p.year_built, p.is_furnished, p.amenities, p.security_deposit, p.rental_type,
        ],
      );
  ...
  private async seedContracts(schema: string) {
    ...
    const properties: { id: number; title: string }[] = await this.dataSource.query(
      `SELECT id, title FROM ${quoteIdent(schema)}.properties ORDER BY id`,
    );

    if (users.length < 2 || properties.length < 2) {
      this.logger.warn('Dev seed: usuarios o propiedades insuficientes para contratos.');
      return;
    }
    ...
    const contracts = [
      {
        ...
        tenant_responsibilities: 'Mantenimiento de equipos, limpieza de áreas comunes.',
        owner_responsibilities: 'Reparaciones estructurales, pintura cada 2 años.',
      },
      {
    ...
    for (const c of contracts) {
      await this.dataSource.query(
        ...
        [
          c.contract_number, c.tenant_id, c.property_id, c.status,
          c.start_date, c.end_date, c.duration_months,
          c.monthly_rent, c.currency, c.deposit_amount, c.payment_day,
          c.late_fee_percentage, c.grace_days, c.payment_method, c.auto_renew, c.jurisdiction,
          c.included_services, c.tenant_responsibilities, c.owner_responsibilities,
        ],
      );
    }
  ...
  private async seedPayments(schema: string) {
    const contracts: { id: number; tenant_id: number; property_id: number; monthly_rent: number }[] =
      await this.dataSource.query(
    ...
    if (contracts[0]) {
      const c = contracts[0];
      payments.push(
        { contract: c, amount: c.monthly_rent * 2, type: 'DEPOSIT', method: 'transferencia', status: 'APPROVED', payment_date: '2024-01-01', due_date: '2024-01-01', reference: 'DEP-001-2024', notes: 'Depósito inicial (2 meses)' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'APPROVED', payment_date: '2024-01-05', due_date: '2024-01-05', reference: 'PAG-ENE-2024', notes: 'Alquiler enero 2024' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'APPROVED', payment_date: '2024-02-05', due_date: '2024-02-05', reference: 'PAG-FEB-2024', notes: 'Alquiler febrero 2024' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'APPROVED', payment_date: '2024-03-05', due_date: '2024-03-05', reference: 'PAG-MAR-2024', notes: 'Alquiler marzo 2024' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'qr_accl', status: 'APPROVED', payment_date: '2024-04-05', due_date: '2024-04-05', reference: 'PAG-ABR-2024', notes: 'Alquiler abril 2024' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'PENDING', payment_date: '2024-05-05', due_date: '2024-05-05', reference: null, notes: 'Alquiler mayo 2024 — pendiente aprobación' },
      );
    }
    ...
    if (contracts[1]) {
      const c = contracts[1];
      payments.push(
        { contract: c, amount: c.monthly_rent * 2, type: 'DEPOSIT', method: 'qr_accl', status: 'APPROVED', payment_date: '2024-03-15', due_date: '2024-03-15', reference: 'DEP-002-2024', notes: 'Depósito inicial' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'qr_accl', status: 'APPROVED', payment_date: '2024-04-10', due_date: '2024-04-10', reference: 'PAG-ABR-CTR2', notes: 'Alquiler abril 2024' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'qr_accl', status: 'REJECTED', payment_date: '2024-05-12', due_date: '2024-05-10', reference: null, notes: 'Comprobante ilegible — rechazado' },
        { contract: c, amount: 450.00, type: 'LATE_FEE', method: 'transferencia', status: 'APPROVED', payment_date: '2024-05-18', due_date: '2024-05-18', reference: 'MORA-001', notes: 'Mora por atraso de mayo' },
      );
    }
    ...
    if (contracts[2]) {
      const c = contracts[2];
      payments.push(
        { contract: c, amount: c.monthly_rent * 2, type: 'DEPOSIT', method: 'transferencia', status: 'APPROVED', payment_date: '2023-01-01', due_date: '2023-01-01', reference: 'DEP-003-2023', notes: 'Depósito inicial' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'APPROVED', payment_date: '2023-01-01', due_date: '2023-01-01', reference: 'PAG-ENE-2023', notes: 'Alquiler enero 2023' },
        { contract: c, amount: c.monthly_rent, type: 'RENT', method: 'transferencia', status: 'APPROVED', payment_date: '2023-06-01', due_date: '2023-06-01', reference: 'PAG-JUN-2023', notes: 'Alquiler junio 2023' },
        { contract: c, amount: c.monthly_rent * 2, type: 'DEPOSIT', method: 'transferencia', status: 'APPROVED', payment_date: '2024-01-10', due_date: '2024-01-01', reference: 'DEV-DEP-003', notes: 'Devolución de depósito al finalizar contrato' },
      );
    }
    ...
    for (const p of payments) {
      await this.dataSource.query(
        ...
        [
          p.contract.tenant_id, p.contract.id, p.contract.property_id,
          p.amount, p.type, p.method, p.status,
          p.payment_date, p.due_date, p.reference, p.notes, adminId,
        ],
      );
    }
  ...
  private async seedMaintenanceRequests(schema: string) {
    const users: { id: number; email: string; role: string }[] = await this.dataSource.query(
    ...
    const contracts: { id: number; tenant_id: number; property_id: number }[] = await this.dataSource.query(
    ...
    const requests = [
      {
        ...
        description: 'Hay una fuga de agua debajo del lavamanos del baño principal. El agua gotea constantemente y está dañando el piso.',
      ...
      },
      {
        ...
        description: 'El tomacorriente doble de la pared principal de la sala no tiene electricidad. Probé con varios aparatos.',
        permission_to_enter: 'YES',
        has_pets: false,
        entry_notes: null,
        status: 'IN_PROGRESS',
      ...
      },
      {
        ...
        description: 'El split del dormitorio principal enciende pero solo expulsa aire caliente. Requiere recarga de gas o revisión técnica.',
      ...
      },
      {
        ...
        description: 'Necesito una copia de la llave del portón principal para mi familiar que vive conmigo.',
        permission_to_enter: 'NOT_APPLICABLE',
        has_pets: false,
        entry_notes: null,
        status: 'CLOSED',
      ...
      },
      {
        ...
        description: 'Las luminarias LED de la cocina parpadean intermitentemente, especialmente al encender el horno.',
        permission_to_enter: 'YES',
        has_pets: false,
    ...
    for (const r of requests) {
      const inserted: { id: number }[] = await this.dataSource.query(
        `INSERT INTO ${quoteIdent(schema)}.maintenance_requests
         ...
         RETURNING id`,
        [
          r.ticket_number, r.request_type, r.category, r.title, r.description,
          r.permission_to_enter, r.has_pets, r.entry_notes, r.status, r.priority,
          r.contract.tenant_id, r.contract.id, r.contract.property_id,
          r.assigned_to, r.current_stage, r.completed_at,
        ],
      );
      ...
      if (r.status === 'IN_PROGRESS' && tecnico) {
        await this.dataSource.query(
          ...
          [reqId, tecnico.id, 'He revisado el tomacorriente. Necesito repuesto (breaker 20A). Lo consigo el martes y termino el miércoles.'],
        );
      }
      ...
      if (r.status === 'COMPLETED' && adminUser) {
        await this.dataSource.query(
          `INSERT INTO ${quoteIdent(schema)}.maintenance_messages
             (maintenance_request_id, user_id, message, send_to_resident, created_at)
           VALUES ($1,$2,$3,true,NOW())`,
          [reqId, adminUser.id, 'Solicitud completada. La fuga fue reparada con éxito. Por favor confirme que todo está en orden.'],
        );
      }
    }

    this.logger.log('  ✔ Solicitudes de mantenimiento semilla creadas (5 solicitudes)');
  }
  ...
  private async seedApplications(schema: string) {
    ...
    const applicantHash = await bcrypt.hash('Inquilino365!', BCRYPT_SALT_ROUNDS);
    for (const ap of [
      { email: 'diego.torrez@gmail.com', name: 'Diego Torrez Salinas', phone: '76611223' },
      { email: 'sofia.rojas@gmail.com', name: 'Sofia Rojas Mendoza', phone: '70077885' },
    ]) {
      await this.dataSource.query(
        `INSERT INTO ${quoteIdent(schema)}."user"
           (email, password, name, phone, role, is_active, created_at, updated_at)
    ...
    const allApplicants: { id: number; email: string }[] = await this.dataSource.query(
    ...
    const byEmail = (email: string) => allApplicants.find((u) => u.email === email)?.id;
        ...
        rental_history: JSON.stringify([
          {
            ...
            reason_for_leaving: 'Cambio de trabajo — busco oficina más céntrica',
            previous_rent_amount: 2500,
          },
        ]),
        references: JSON.stringify([
          { name: 'Ing. Pedro Alvarado', relationship: 'Jefe directo', phone: '70099001' },
          { name: 'Dra. Carla Vidal', relationship: 'Colega', phone: '70099002' },
        ]),
        additional_notes: 'Necesito la oficina para instalar mi empresa importadora. Pagaré con factura mensualmente.',
        admin_feedback: 'Documentación completa. Ingresos verificados. Pendiente llamada al empleador.',
      },
      {
        property_id: properties[2].id,
        ...
        rental_history: JSON.stringify([]),
        references: JSON.stringify([
          { name: 'Lic. Marco Suárez', relationship: 'Cliente frecuente', phone: '70088001' },
        ]),
        additional_notes: 'Primera vez alquilando oficina propia. Puedo entregar cartas de clientes como respaldo de ingresos.',
        admin_feedback: null,
      },
      {
        property_id: properties[3].id,
        ...
        rental_history: JSON.stringify([]),
        references: JSON.stringify([
          { name: 'Dr. Ernesto Chávez', relationship: 'Padre', phone: '70011223' },
        ]),
        additional_notes: 'Estudiante responsable. El alquiler lo cubre mi familia. Pueden verificar con mis padres.',
        admin_feedback: 'Aprobada. Buen perfil para el estudio. Contrato generado como CTR-2023-001.',
      },
    ];
    ...
    for (const a of applications) {
      ...
      await this.dataSource.query(
        ...
        [
          a.property_id, a.applicant_id, a.status,
          a.personal_data, a.employment_data,
          a.rental_history, a.references,
          a.additional_notes, a.admin_feedback,
        ],
      );
    }

    this.logger.log('  ✔ Solicitudes de alquiler semilla creadas (3 solicitudes)');
  }
}
